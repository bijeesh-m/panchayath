...
module.exports.fetchAppliedSpecificCert = async (req, res, next) => {
    try {
        ...
        const certDetails = await appliedBrithCertModel.findOne({ _id: certId }).populate({
            path: "userId",
            model: "user",
            select: "userName email username",
        });
...
module.exports.verifyCertificate = async (req, res, next) => {
    try {
        ...
        if (specificCert) {
            await appliedBrithCertModel.updateOne({ _id: certId }, { $set: { certStatus: true } });
        }
        return res.json({
...
module.exports.verifyMarriageCert = async (req, res, next) => {
    try {
        ...
        if (specificCert) {
            await marriageCertModel.updateOne({ _id: certId }, { $set: { certStatus: true } });
        }
        return res.json({
            message: "Verified successfully",
            status: true,
...
module.exports.removeCertificate = async (req, res, next) => {
    ...
    try {
        await certificateModel.findByIdAndDelete(certId);
        res.status(200).json({ message: "Certificate deleted!" });
    } catch (error) {
        res.status(400).json({ message: "Certificate deletion failed!" });
    }
};
