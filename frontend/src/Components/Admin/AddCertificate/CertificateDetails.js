...
import { addCertificate, getAllCertificate, removeCertificate } from "../../../Services/adminApi";
...
function CertificateDetails() {
    ...
    const validationSchema = Yup.object({
        certName: Yup.string().min(3, "*Name must be at least 3 characters long").required("* This field is required"),
    });
    ...
    const handleRemoveCertificate = async (certId) => {
        try {
            const response = await removeCertificate(certId);
                            ...
                            <tbody>
                                {certificate.map((value, index) => (
                                    <tr key={value._id}>
                                        ...
                                        <button
                                            onClick={() => handleRemoveCertificate(value._id)}
